...
public final class AlarmInstance implements AlarmContract.InstancesColumns {
    /**
...
//    private static final int ALARM_STATE_INDEX = 10;
    ...
    private static final int COLUMN_COUNT = ALARM_ID_INDEX + 1;
    ...
    public static List<AlarmInstance> getInstancesByInstanceIdAndState(
            ContentResolver contentResolver, long alarmInstanceId) {
        return getInstances(contentResolver, _ID + "=" + alarmInstanceId);
    }
    ...
    public AlarmInstance(Cursor c, boolean joinedTable) {
        if (joinedTable) {
            mId = c.getLong(AlarmInfo.ID_INDEX);
            mHour = c.getInt(AlarmInfo.HOUR_INDEX);
            mMinute = c.getInt(AlarmInfo.MINUTES_INDEX);
            mLabel = c.getString(AlarmInfo.LABEL_INDEX);
            mVibrate = c.getInt(AlarmInfo.VIBRATE_INDEX) == 1;
        } else {
